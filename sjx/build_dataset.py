...
def build_examples(image_dir, mask_dir, json_dir, N_slices, N_neg_slices, seed,
    ...
    np.random.seed(seed)
    #processor = CLIPSegProcessor.from_pretrained("CIDAS/clipseg-rd64-refined", cache_dir="./CLIP/hf_cache")#下载huggingface上的模型
    processor = CLIPSegProcessor.from_pretrained("./CIDAS_clipseg-rd64-refined", cache_dir="./CLIP/hf_cache")#使用本地模型
    ...
    for img_path, msk_path in tqdm(zip(image_paths, mask_paths), total=len(image_paths), desc="cases"):
        ...
        for organ, idx in info["label_mapping"].items():
            ...
            for slice_i in np.concatenate((sel, sel_neg)):
                ...
                img_slice = ((img_slice - hu_clip_min[organ]) / (hu_clip_max[organ] - hu_clip_min[organ])) * 255   # all values in [0, 255]
